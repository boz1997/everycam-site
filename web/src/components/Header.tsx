...
export function Wordmark() {
  return (
    <span className="sc-h-wordmark">
      Share<i>Cam</i>
    </span>
  );
}

export function Header<L extends string>({ lang, langs, langLabel, onLang, languageName, homeHref, right, tag }: HeaderProps<L>) {
  const brand = (
    <>
      <Mark />
      <Wordmark />
  ...
  return (
    <header className="sc-h">
      <div className="sc-h-inner">
        {homeHref ? (
          <a className="sc-h-brand" href={homeHref} aria-label="ShareCam">
            {brand}
          </a>
        ) : (
          <span className="sc-h-brand">{brand}</span>
        )}
        {tag && <span className="sc-h-tag">{tag}</span>}
        <span className="sc-h-space" />
        {right}
        <label className="sc-h-lang">
          <span className="sc-h-sr">{languageName}</span>
          <select value={lang} onChange={(e) => onLang(e.target.value as L)}>
            {langs.map((l) => (
